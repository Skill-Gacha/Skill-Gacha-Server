...
![image (6)](https://github.com/user-attachments/assets/07abb8c5-01d1-4f47-8c47-85d9f5950321)
...
3. **추상화 통한 일관성 확보**: 모든 상태는 동일한 추상 클래스를 상속하므로, 상태별 진입(`enter`) 시점, 입력 처리(`handleInput`) 시점이 명확하게 보장됩니다. 이는 팀 개발 및 지속적
   기능 확장 상황에서 특히 유용합니다.
...
- 불, 물, 풀, 전기, 땅 각 속성에 대한 저항값을 설정하고 상성 관계를 통한 데미지 계산
...
- 3명 파티 매칭에 Bull Queue 활용, 실시간 상태 브로드캐스팅으로 협력 플레이 강화
