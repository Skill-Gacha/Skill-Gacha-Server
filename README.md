...
# <div align="center">가차 없는 스킬 가챠</div>
...
이에 비해 상태 머신 패턴을 도입하면, 각 **상태**(State)가 독립적으로 존재하고, **입력**(Input)에 따라 **명확한 전이**(Transition)가 일어나며, **추상 클래스**를 통한 공통
메서드(`enter()`, `handleInput()`) 구현으로 확장성을 갖출 수 있습니다.
...
- **도메인별 상태 상속 (`DungeonState`, `ActionState`, `SkillChoiceState` 등)**:  
  던전 컨텍스트를 관리하는 `DungeonState`, 전투 로직을 처리하는 `ActionState`, 스킬 선택을 담당하는 `SkillChoiceState` 등 **도메인 및 기능별로 세분화된 상태 클래스**를
  구성합니다.  
...
3. **추상화를 통한 일관성 확보**: 모든 상태는 동일한 추상 클래스를 상속하므로, 상태별 진입(`enter`) 시점, 입력 처리(`handleInput`) 시점이 명확하게 보장됩니다. 이는 팀 개발 및 지속적
   기능 확장 상황에서 특히 유용합니다.
...
- 던전 클리어 시 난이도별로 다른 등급의 스킬을 확률적으로 획득
- 강화 시 등급별 재화 요구량과 성공/실패 확률이 변하며, 고등급 강화 시 등급 하락 가능성으로 리스크-리턴 밸런스 제공

### 속성 시스템

- 불, 물, 풀, 전기, 땅 각 속성에 대한 저항값을 설정하고 상성 관계를 통한 대미지 계산
- 단순 힘 싸움이 아닌 속성 이해와 전략적 스킬 선택이 전투 결과에 큰 영향

### 아이템 시스템

- 중앙 집중식 아이템 관리 구조를 갖춰 유지보수성 확보
- 특정 조건 하에서 사용 가능하거나, 확률적으로 다른 효과가 발동되는 포션으로 전투 변수 극대화

### PVP (1:1 대전)

- 승부 결과를 기반으로 랭크 점수 산정 및 상위 랭커 노출로 경쟁심 고취
- 제한 시간 내 턴 진행, 비정상 종료 처리 등 안정적이고 공정한 전투 환경 조성

### BOSS RAID

- 3인 파티 매칭에 Bull Queue 활용, 실시간 상태 브로드캐스팅으로 협력 플레이 강화
- 보스 체력 상태에 따라 공격 패턴 및 속성 활용 변동, 단계별 전략 수립 요구
...
- **해결**: import 경로의 대소문자 구분을 일관되게 하기 위해, 서브패스 임포트 등으로 경로 통일.
...
이를 개선하는 방법으로 다음의 방법을 생각해 볼 수 있습니다.
