...
// 핸들러 레벨의 락 추가
const playerResponseLock = new AsyncLock();
...
export const cBossPlayerResponseHandler = async ({ socket, payload }) => {
  ...
  try {
    await playerResponseLock.acquire('cBossPlayerResponseHandler', async () => {
    ...
    logger.error('cBossPlayerResponseHandler: 오류입니다.', error);
  }
};
