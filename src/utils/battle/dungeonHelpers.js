...
export function sendMonsterHpUpdate(socket, monster) {
  try {
    socket.write(createResponse(PacketType.S_SetMonsterHp, { monsterIdx: monster.monsterIdx, hp: monster.monsterHp }));
  } catch (error) {
...
export function sendMonsterAction(socket, monsterIdx, actionSet) {
  const payload = { actionMonsterIdx: monsterIdx, actionSet };
...
export function sendScreenText(socket, msg, typingAnimation = false) {
  const screenText = { msg, typingAnimation };
